		...
		function add_meta_box( $post_type = null ) {
			if ( ! empty( $post_type ) ) {
				...
				if ( ! in_array( $post_type, $non_layout_post_types ) )
					add_meta_box( 'layout_meta_box', __( 'Custom Builder Layout', 'it-l10n-Builder' ), array( $this, 'render_box' ), $post_type, 'side', 'low' );
			}
			else {
				add_meta_box( 'layout_meta_box', __( 'Custom Builder Layout', 'it-l10n-Builder' ), array( $this, 'render_box' ), 'post', 'side' );
				add_meta_box( 'layout_meta_box', __( 'Custom Builder Layout', 'it-l10n-Builder' ), array( $this, 'render_box' ), 'page', 'side' );
			}
		}
...
?>
	...
	<p><?php printf( __( 'Choosing a Custom Builder Layout causes the chosen Layout to be used when viewing this %1$s. It will override any custom Views you have configured in the <a href="%2$s">Views Editor</a>.', 'it-l10n-Builder' ), $type, admin_url( 'admin.php?page=layout-editor&editor_tab=views' ) ); ?></p>
